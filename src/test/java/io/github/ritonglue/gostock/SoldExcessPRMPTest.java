...
public class SoldExcessPRMPTest {
	...
	public void testOneSellAndBuy2Excess() {
		...
		list.add(TradeWrapper.buy(createQuantity(7), createMoney("13"), a));
		list.add(TradeWrapper.buy(quantity, amount, b));
		list.add(TradeWrapper.sell(quantitySell, c));
		...
		List<TradeWrapper> orphanSells = manager.getOrphanSells();
		Assert.assertEquals(1, orphanSells.size());
		Assert.assertEquals(d, orphanSells.get(0).getSource());
		Assert.assertEquals(createQuantity("2"), orphanSells.get(0).getQuantity());

		Position position = closed.get(0);
		Assert.assertTrue(position.isClosed());
		Assert.assertEquals(quantitySell, position.getQuantity());
		Assert.assertEquals(createMoney("45.20"), position.getAmount());
		SourceTest sell = position.getSell(SourceTest.class);
		Assert.assertEquals(c, sell);

		position = closed.get(1);
		Assert.assertTrue(position.isClosed());
		Assert.assertEquals(createQuantity(6), position.getQuantity());
		Assert.assertEquals(createMoney("67.80"), position.getAmount());
